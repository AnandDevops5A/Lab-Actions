...
const NavbarContent = () => {
  ...
  const { user, getUserFromContext,MALIK } = useContext(UserContext);
  ...
  const navItems = useMemo(
    () => [
      ...
      },
      {
        name: "leaderboard",
        href: "/Leaderboard",
        icons: <LucideTrophy className={iconClass} />,
      },
      {
        name: MALIK ? "Admin" : "Contact",
        href: MALIK ? "/admin" : "/?scroll=contact-page",
        icons: MALIK ? (
      ...
      {
        name: user ? "My Profile" : "Login",
        href: user ? "/player" : "/auth",
        icons: user ? (
            ...
            {navItems.map((item, idx) => (
              ...
              >
                <span className="relative inline-block">
                  ...
                  <span
                    className={`absolute left-0 -bottom-1 h-0.5 w-full bg-linear-to-r from-red-400 to-yellow-400 transform ${activeItem === item.name ? "scale-x-100" : "scale-x-0"} group-hover:scale-x-100 transition-transform origin-left`}
                  ></span>
                </span>
              </Link>
            ))}
