...
const Achievement = ({ matchHistory }) => {
  ...
  return (
    <div
      className={`p-6 rounded-xl border shadow-lg transition-colors duration-300 ${isDarkMode ? "bg-gray-800 border-red-700/50 shadow-gray-700/50" : "bg-white border-red-200 shadow-gray-200"}`}
    >
      <h2
        className={`text-2xl font-bold uppercase mb-6 ${isDarkMode ? "text-red-500" : "text-red-600"}`}
      >
        Match Feed
      </h2>
        ...
        {sortedMatchHistory && sortedMatchHistory.length > 0 ? (
          sortedMatchHistory.map((match, idx) => (
            ...
            >
              <div>
                <p
                  className={`font-semibold text-lg ${isDarkMode ? "text-slate-100" : "text-gray-900"}`}
                >
