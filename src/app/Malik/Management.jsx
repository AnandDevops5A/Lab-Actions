...
const TournamentDetailsModal = dynamic(() => import('./TournamentDetailsModal'), {
  loading: () => <div className="text-green-400">Loading...</div>,
  ssr: false,
});
...
const TournamentManagement = ({ tournaments, refreshData }) => {
  ...
  const filteredTournaments = useMemo(() => {
    ...
    return filtered;
  }, [tournaments, searchTerm, statusFilter, platformFilter, sortBy, sortOrder, now]);
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    try {
      ...
      if (successCount === selectedTournaments.length) {
        ...
        errorMessage(`Failed to delete ${selectedTournaments.length - successCount} tournament(s)`);
      }
    } catch (error) {
  ...
  const stats = useMemo(() => {
    if (!tournaments) return { total: 0, upcoming: 0, completed: 0, totalPrize: 0 };
  ...
  const deleteTournament = useCallback(async (tournamentId) => {
    const confirmed = await confirmMessage("Are you sure you want to delete this tournament?", "Delete Tournament");
    if (!confirmed) return;

    try {
      const response = await fetch(`${BASE_URL}/tournament/delete/${tournamentId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className={`p-4 rounded-lg border ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Total Tournaments</p>
        ...
        <div className={`p-4 rounded-lg border ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Upcoming</p>
              <p className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{stats.upcoming}</p>
            </div>
            <Calendar className={`h-8 w-8 ${isDarkMode ? 'text-amber-400' : 'text-amber-600'}`} />
          </div>
        </div>
        ...
        <div className={`p-4 rounded-lg border ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{stats.completed}</p>
            </div>
        ...
        <div className={`p-4 rounded-lg border ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Total Prize Pool</p>
              <p className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>₹{stats.totalPrize.toLocaleString()}</p>
            </div>
  ...
  {!showAddForm ? (<>
    {/* Controls */}
      <div className={`animate-slideInDown p-4 rounded-lg border ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
   ...
   {/* Tournaments Table */}
        <div className={`animate-slideInDown rounded-lg border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
          ...
          <table className="w-full">
            ...
            <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
              {filteredTournaments.map((tournament, index) => {
                ...
                return (
                  <tr key={tournament.id} className={`hover:bg-gray-50 dark:hover:bg-gray-700 ${selectedTournaments.includes(tournament.id) ? 'bg-green-50 dark:bg-green-900/20' : ''}`}>
                    ...
                    <td className="px-4 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${statusInfo.color}`}>
                        <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${statusInfo.dotColor}`}></span>
                        {statusInfo.label}
                      </span>
                    </td>
                    <td className="px-4 py-4">
                      <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium border ${isDarkMode ? 'bg-gray-700 border-gray-600 text-gray-300' : 'bg-gray-100 border-gray-300 text-gray-700'}`}>
                        {tournament.platform}
                      </span>
                    </td>
                    <td className="px-4 py-4">
        ...
        {filteredTournaments.length === 0 && (
          <div className="text-center py-12">
            <Trophy className={`mx-auto h-12 w-12 ${isDarkMode ? 'text-gray-400' : 'text-gray-400'}`} />
            <h3 className={`mt-2 text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>No tournaments found</h3>
