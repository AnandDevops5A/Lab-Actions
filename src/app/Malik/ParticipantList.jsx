...
import { confirmMessage, errorMessage, successMessage } from "../Library/Alert";

const ParticipantList = ({ selectedTournament, isDarkMode, refreshData, onBack }) => {
  const [editingParticipant, setEditingParticipant] = useState(null);
  const [editData, setEditData] = useState({ rank: '', investAmount: '', winAmount: '' });
  const [loading, setLoading] = useState(false);
  const now = Date.now();

  const getEditableFields = (tournament) => {
    if (new Date(tournament.dateTime).getTime() > now) {
      return { canEditInvest: true, canEditRank: false, canEditWin: false, type: 'upcoming' };
    } else {
      return { canEditInvest: false, canEditRank: true, canEditWin: true, type: 'completed' };
    }
  };
  ...
  const getRankColor = (rank) => {
    switch(rank) {
      case 1: return isDarkMode ? "text-yellow-400" : "text-yellow-600";
      case 2: return isDarkMode ? "text-gray-400" : "text-gray-600";
      case 3: return isDarkMode ? "text-amber-400" : "text-amber-600";
  ...
  const handleSaveParticipant = async (participantId, tournamentId, tournament) => {
    const editableFields = getEditableFields(tournament);
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8082/leaderboard/update/${participantId}`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        const message = editableFields.type === 'upcoming'
  ...
  const approveParticipant = async (participantId) => {
    const confirmed = await confirmMessage("Approve this participant's results?", "Confirm Approval");
    if (!confirmed) return;

    setLoading(true);
    try {
      const response = await fetch(`http://localhost:8082/leaderboard/approve/${participantId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    <div className={`p-6 rounded-lg border ${isDarkMode ? 'bg-gray-900/50 border-gray-700' : 'bg-white border-gray-200'}`}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className={`text-2xl font-bold flex items-center gap-3 ${isDarkMode ? 'text-green-400' : 'text-green-600'}`}>
        ...
        <div className="flex items-center gap-4">
          <div className={`px-4 py-2 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
            <div className="text-sm">
              <span className={isDarkMode ? 'text-gray-400' : 'text-gray-600'}>Prize Pool: </span>
          ...
          <div className={`px-4 py-2 rounded-lg ${
            new Date(selectedTournament.dateTime).getTime() > now
            ...
            {new Date(selectedTournament.dateTime).getTime() > now ? 'UPCOMING' : 'COMPLETED'}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {selectedTournament.leaderBoard && selectedTournament.leaderBoard.length > 0 ? (
          <div className="space-y-4">
            {selectedTournament.leaderBoard.map((participant) => (
              <div
                key={participant.id}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-4">
                    {editingParticipant === participant.id ? (
                      <div className="flex items-center gap-2">
                        <div className="flex items-center gap-2">
                          {getEditableFields(selectedTournament).canEditRank && (
                            <input
                              type="number"
                              placeholder="Rank"
                              value={editData.rank}
                          ...
                          {getEditableFields(selectedTournament).canEditInvest && (
                            <input
                              type="number"
                              placeholder="Invest Amount"
                              value={editData.investAmount}
                              onChange={(e) => setEditData(prev => ({ ...prev, investAmount: e.target.value }))}
                          ...
                          {getEditableFields(selectedTournament).canEditWin && (
                            <input
                              ...
                              onChange={(e) => setEditData(prev => ({ ...prev, winAmount: e.target.value }))}
                              className={`w-24 px-2 py-1 text-sm rounded border ${
                                isDarkMode
                                  ? 'bg-gray-700 border-gray-600 text-white'
                                  : 'bg-white border-gray-300 text-gray-900'
                        ...
                        <button
                          onClick={() => handleSaveParticipant(participant.id, selectedTournament.id, selectedTournament)}
                          disabled={loading}
                          className={`p-1 rounded ${
                            isDarkMode ? 'hover:bg-green-600' : 'hover:bg-green-500'
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => handleEditParticipant(participant, selectedTournament)}
                            className={`p-2 rounded-lg transition-colors ${
                              isDarkMode ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-100 text-gray-600'
                            }`}
                            title={
                          ...
                          {!participant.isApproved && (
                            <button
                              ...
                              disabled={loading}
                              className={`p-2 rounded-lg transition-colors ${
                                isDarkMode ? 'hover:bg-green-700 text-green-400' : 'hover:bg-green-100 text-green-600'
                              }`}
